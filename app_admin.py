...
# Full-width content; keep table nowrap for admin so you can resize columns & scroll
st.markdown(
    f"""
    <style>
...
def load_df(engine: Engine) -> pd.DataFrame:
    ...
    for col in ["contact_name", "phone", "address", "website", "notes", "keywords", "service", "created_at", "updated_at", "updated_by"]:
        if col not in df.columns:
            df[col] = ""
    # short previews for browsing table only (admin can still open full rows in edit form)
