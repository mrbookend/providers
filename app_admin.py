...
page_max_width_px = 2300
sidebar_state = "expanded"  # "collapsed" or "expanded"

# --- Read-only help text ---
READONLY_HELP_TITLE = "Provider Help / Tips"
READONLY_HELP_MD = """
- Type a word or part of a word (e.g., `plumb`).
- Sort by clicking a column header.
- Website links open in a new tab.
"""

# --- Debug ---
ADMIN_DEBUG = "false"  # set to "true" to show readonly debug block
...
> Notes:
>
> * We intentionally removed streamlit-aggrid to avoid prior React errors and chunk CSS issues.
> * If you later want AgGrid, add `streamlit-aggrid==0.3.4.post3` and wire it up carefully.
...
* Create a database and an auth token in Turso.
* Set `TURSO_DATABASE_URL` and `TURSO_AUTH_TOKEN` in `.streamlit/secrets.toml`.
* Run the admin app once to auto-create tables: `python -m streamlit run app_admin.py`.
* (Optional) seed libs: in admin, add categories/services; or run `sqlite3` with `seed.sql` against your DB.
...
* Create new app, point to your repo, branch `main`, file `app_readonly.py` (and/or `app_admin.py`).
* In **App secrets**, paste the same keys from `.streamlit/secrets.toml`.
* Deploy. If Turso creds are wrong or down, the app will fall back to `vendors.db` file.

## 6) Daily use

* Use **Admin** to add/edit vendors and manage categories/services.
